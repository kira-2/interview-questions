...
// Как будешь работать с древовидной структурой данных?
// Почему состояние в компоненте нельзя напрямую менять в React?
// Rest Api
// JWT tokens

// Спрашивали по типам данных, про области видимости, про виды объявления, методом массивов, Промиссам, какие есть состояния у него, про контекст, про bind, call, apply, Про то. что такое HTTP, какие методы в нем есть. Работал ли с генераторами, Работал ли с unit тестированием, дальше спрашивали про реакт, работал ли с классовыми компонентами, о жизненых этапах компонента, правила использования хуков (не ответил!), писал ли кастомные хуки, про State спрашивали, про connect тоже спрашивали (mapStateToProps, mapDispatchToProps), использовал ли я сборщики, Electron, как сделать State зависимый компонент, что такое Virtual Dom и в чем его концепция."
// JS: замыкание, хоистинг, контекст функций, промисы, геттеры и сеттеры сss: shadowDOM, вес селекторов, flex и order react: рассказала про все хуки и редакс, спрашивали useCallback useDebugValue

// Event loop, погружение-захват-всплытие, ООП, agile-методологии, stopProganation, микротаски макротаски, виртуальный дом дерево (почему он копируется - ответ: типа посмотри в консоли как ведет себя querySelector, а виртальный дом не такой - он легковесный), умные глупые компоненты. Задание что выведет консоль: const bar = {a: 1}; const foo = bar; bar.a = 2; console.log(foo, bar) (a: 2; в обоих случаях так как ссылка одна)
...
// Промисы, async/await, callback hell(много было ещё вопросов, не все запомнил)"
