...
1. Информационные 100–199 (например, смена протокола с HTTP на WebSocket).
1. Успешные 200–299
1. Перенаправления 300–399 (сервер просит перейти на другую страницу).
1. Клиентские ошибки 400–499 (запрошен несуществующий адрес, переданы некорректные данные и т.д.).
1. Серверные ошибки 500–599 (сервер не смог обратиться к БД или другая внутренняя ошибка).
